...
def test_fixed_scope_keeps_original_task_signatures(tmp_path):
    from experiments.sd_membership_sft.tests.test_qwen_audit_matrix import settings
...
def test_fixed_summary_preserves_existing_outputs_and_excludes_natural_cost(tmp_path):
    import json
    from experiments.sd_membership_sft.tests.test_qwen_audit_matrix import tasks_at, write_example_result
    ...
    assert (root / 'fixed_only_summary' / 'RESULTS.csv').exists()
