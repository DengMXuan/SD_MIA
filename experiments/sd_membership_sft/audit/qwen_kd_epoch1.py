"""Qwen3 epoch-1 auxiliary-KD main method: three datasets x three condition seeds.

This entry point never schedules baselines, member drafts, or language-model
training. The shared coordinator supplies per-GPU locks and safe resumption.
"""
import argparse
import json
from pathlib import Path

from experiments.paths import AUDITS, audit_reports, audit_executions
from experiments.cross_model_audit import engine
from experiments.shared.audit import reporting, scheduler
from experiments.shared.audit.config import audit_settings
from experiments.shared.audit.devices import check_gpus
from experiments.shared.audit.evaluation import _validate_output
from experiments.shared.audit.provenance import read_result
from experiments.shared.models.registry import MODEL_PAIRS

BENCHMARKS = ('wikitection', 'newstection', 'arxivtection')
SEEDS = (1919, 1949, 1978)
ROLE = 'draft_auxiliary_distilled'
METHOD = 'main_fixed_sparse_positive'
OUTPUT_ROOT = AUDITS / 'qwen_kd_epoch1_condition_seed_v1/tasks'


def validate_scope(task):
    """Reject a saved worker task that expands this experiment's fixed scope."""
    condition = task['condition']
    if (task.get('model_pair') != 'qwen3' or condition.get('model_pair') != 'qwen3'
            or task.get('kind') != 'main' or task.get('draft_role') != ROLE
            or task.get('protocol') != 'fixed' or task.get('methods') != [METHOD]
            or condition.get('benchmark') not in BENCHMARKS or condition.get('epoch') != 1
            or condition.get('condition_seed') not in SEEDS):
        raise ValueError('this experiment requires Qwen3 epoch 1 auxiliary-KD main tasks only')
    seed = condition['condition_seed']
    if (task['settings'].get('audit_seed') != seed
            or task['settings'].get('seed_policy') != 'condition_v1'):
        raise ValueError('audit, auxiliary partition and AUC seeds must match the condition seed')
    _validate_output(Path(task['run_dir']).resolve(), Path(task['output']).resolve())


def make_tasks(output_root=OUTPUT_ROOT, *, detector_epochs=30, model_root=None):
    spec = MODEL_PAIRS['qwen3']
    model_root = Path(model_root or spec.run_root).resolve()
    output_root = Path(output_root).resolve()
    # Validate the coordinator/report roots too, before any directory creation.
    _validate_output(model_root, output_root)
    _validate_output(model_root, audit_reports(output_root))
    _validate_output(model_root, audit_executions(output_root))
    candidates = engine.make_tasks(model_root, output_root, BENCHMARKS, [1], SEEDS,
                                   audit_settings(detector_epochs=detector_epochs), model_pair=spec.name)
    tasks = [task for task in candidates if task['kind'] == 'main' and task['draft_role'] == ROLE]
    if len(tasks) != 9 or len({task['output'] for task in tasks}) != 9:
        raise ValueError('expected nine distinct main-method configurations')
    for task in tasks:
        validate_scope(task)
    return tasks


def inspect_task(task):
    validate_scope(task)
    return engine.inspect_task(task)


def summarize(tasks, output_root):
    for task in tasks:
        validate_scope(task)
        _validate_output(Path(task['run_dir']).resolve(), audit_reports(output_root))
    spec = MODEL_PAIRS['qwen3']
    return reporting.summarize(
        tasks, audit_reports(output_root), methods=(METHOD,), baseline_methods=(),
        describe=lambda task: (dict(model_pair=spec.name, adapter=spec.adapter,
                                    target_model=spec.target, draft_model=spec.draft), (ROLE,)),
        title='Qwen3 epoch-1 KD main method (condition seeds)', read_result=read_result,
        execution_root=audit_executions(output_root),
    )


def execute_worker(task):
    validate_scope(task)
    engine.execute_worker(task)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('command', choices=('dry-run', 'status', 'run', 'summarize', 'worker'))
    parser.add_argument('--output-root', type=Path, default=OUTPUT_ROOT,
                        help='task root for a new batch; never reuse historical audit directories')
    parser.add_argument('--gpus', nargs='+', type=int, default=[0])
    parser.add_argument('--detector-epochs', type=int, default=30)
    parser.add_argument('--task-file', type=Path)
    args = parser.parse_args()
    if args.command == 'worker':
        if args.task_file is None:
            parser.error('worker requires --task-file')
        execute_worker(json.loads(args.task_file.read_text()))
        return
    if args.detector_epochs < 1:
        parser.error('detector epochs must be positive')
    if len(args.gpus) != len(set(args.gpus)) or any(gpu < 0 for gpu in args.gpus):
        parser.error('choose distinct nonnegative GPU indices')
    args.output_root = args.output_root.resolve()
    try:
        tasks = make_tasks(args.output_root, detector_epochs=args.detector_epochs)
    except ValueError as error:
        parser.error(str(error))
    if args.command in ('dry-run', 'status'):
        states = {task['id']: inspect_task(task) for task in tasks}
        print(json.dumps(dict(
            model_pair='qwen3', target_epoch=1, draft_role=ROLE, method=METHOD,
            worker_tasks=len(tasks), expected_method_rows=len(tasks), baseline_tasks=0,
            data_counts=dict(member=2000, nonmember=2000, audit_auxiliary=600),
            auxiliary_partitions=dict(train=320, validation=80, calibration=200),
            seed_mapping=[dict(training_data_seed=seed, main_seed=seed,
                               auxiliary_split_seed=seed, auc_bootstrap_seed=seed) for seed in SEEDS],
            output_root=str(args.output_root), summary_directory=str(audit_reports(args.output_root)),
            states=states,
        ), indent=2))
        if any(s['state'] in ('pending', 'invalid', 'stale') for s in states.values()):
            raise SystemExit(2)
        return
    success = True
    if args.command == 'run':
        success = scheduler.run_tasks(tasks, args.output_root, args.gpus, inspect_task=inspect_task,
                                      check_gpus=check_gpus, worker_module=__spec__.name)
    result = summarize(tasks, args.output_root)
    print(json.dumps({key: result[key] for key in ('complete', 'expected_rows', 'completed_rows', 'errors')}))
    if not success or not result['complete']:
        raise SystemExit(2)


if __name__ == '__main__':
    main()
