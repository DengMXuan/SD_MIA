#!/usr/bin/env bash
# Does not train language models. Use dry-run before run; see --help.
set -euo pipefail
SD_AUDIT_SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
cd "$SD_AUDIT_SCRIPT_DIR/../.."
# Scheduling only: tolerate small CUDA/background allocations (MiB).
# Keep this override outside the fingerprinted experiment Python sources so
# existing reports, detector caches and partial trajectories remain resumable.
# Override with SD_AUDIT_GPU_MAX_USED_MIB=2048 if needed; default is 1024.
# This entry now schedules fixed-candidate + baseline only. Natural-SD outputs
# stay untouched; filtered summaries go to fixed_only_summary/.
exec "${SD_AUDIT_PYTHON:-.venv/bin/python}" -u - "$@" <<'PY'
import argparse
import json
import os
...
from experiments.sd_membership_sft import qwen_audit_matrix as matrix
...
def main():
    ...
    from experiments.sd_membership_sft.sd_protocol import resolve_starts
