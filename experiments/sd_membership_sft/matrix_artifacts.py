...
from .protocol_archive import atomic_npz
...
def runtime_files():
    roots = (ROOT / "experiments/sd_membership_sft", ROOT / "experiments/baseline")
...
def sources_for(run_dir, checkpoint_roles):
    ...
    return {"files": [{"path": str(p), "sha256": sha256_file(p)} for p in files],
