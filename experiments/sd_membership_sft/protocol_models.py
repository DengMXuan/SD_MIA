...
from .generalization import load_run_config, load_finetuned_model, load_draft_model
from .scoring_common import DeploymentScoringRecords, prepare_deployment_scoring_records, resolve_checkpoint_path
from .deployment_archive import checkpoint_fingerprint, sha256_file
from .sd_protocol import FrozenAdapter
...
DRAFT_ROLES = ("draft_auxiliary_distilled", "draft_member_sft")


def checkpoint_paths(run_dir: Path, kind: str, draft_role: str = DRAFT_ROLES[0]) -> tuple[Path, Path]:
    if draft_role not in DRAFT_ROLES or (kind != "plain" and draft_role != DRAFT_ROLES[0]):
    ...
    draft = (resolve_checkpoint_path(run_dir, draft_role) if kind == "plain"
             else run_dir / "heads" / "auxiliary_head")
    if not draft.is_dir():
        raise FileNotFoundError(f"matching auxiliary checkpoint not ready: {draft}")
    if kind != "plain":
        from .drafts.common import checkpoint_complete
...
def prepare_records(run_dir: Path, kind: str):
    if kind == "plain":
        cfg, records = prepare_deployment_scoring_records(run_dir)
        return cfg, records
    from .drafts.common import PAIR_MODELS, tokenizer_for, tokenizer_source_for
    from .splits import build_controlled_split_from_shared_manifest, pool_path
...
def load_adapter(run_dir: Path, kind: str, device: str, draft_role: str = DRAFT_ROLES[0]):
    ...
    if kind == "plain":
        ...
        from .drafts.heads import load_eagle3_speculator
        draft = load_eagle3_speculator(str(draft_path), device)
    else:
        from .drafts.heads import load_mtp_speculator
