...
from .audit_runtime import ROOT, _write_json
from .deployment_archive import sha256_file
from .matrix_artifacts import digest, read_result, sources_for
from .matrix_baselines import BASELINE_DEFAULTS
from .matrix_main import MAIN_METHODS

ROLES = ("draft_auxiliary_distilled", "draft_member_sft")
ALL_METHODS = (*MAIN_METHODS["fixed"], *MAIN_METHODS["natural"], *METHODS)
DEFAULT_MODEL_ROOT = ROOT / "experiments/results/sft_runs/unified_matrix_audit600_v2/model_pairs/qwen3"
DEFAULT_OUTPUT_ROOT = ROOT / "experiments/results/sft_runs/qwen_audit_matrix_v1"
                ...
                while not stop.is_set():
                    ...
                    with (attempt / "worker.log").open("w") as log:
                        process = subprocess.Popen(
                            [sys.executable, "-u", "-m", "experiments.sd_membership_sft.qwen_audit_matrix",
...
def execute_worker(task):
    import torch
    from .protocol_models import prepare_records
    from .matrix_baselines import run_baselines
    from .matrix_main import run_main
...
def main():
    ...
    from .sd_protocol import resolve_starts
