...
from experiments.cross_model_audit import engine, models, main_method
from experiments.cross_model_audit.artifacts import digest, read_result, sources_for, runtime_files
...
from experiments.sd_membership_sft.audit.matrix_artifacts import runtime_files as legacy_sources
...
from experiments.sd_membership_sft.tests.test_qwen_audit_matrix import (
    settings, prepared_records, write_example_result,
)
...
def test_head_sources_track_selected_weights_and_reject_tampering(head_run, monkeypatch):
    task, run=head_run
    import experiments.cross_model_audit.artifacts as artifacts
    ...
    from experiments.sd_membership_sft.audit.matrix_artifacts import check_sources_light
...
def test_model_loader_routes_each_head_to_the_matching_target(head_run, monkeypatch, kind, pair):
    _,run=head_run
    from experiments.sd_membership_sft.drafts import heads
    ...
    monkeypatch.setattr(models,'FrozenAdapter',lambda t,d,k,dev:SimpleNamespace(target=t,kind=k))
...
def test_worker_routes_record_reconstruction_and_weight_roles(tmp_path, monkeypatch, pair, kind, baseline):
    from experiments.sd_membership_sft.audit import matrix_baselines
