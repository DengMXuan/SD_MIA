...
def run_main(task, device, cfg, prepared, sources):
    ...
    if archive.exists() and archive.with_suffix(".npz.json").exists():
        ...
        if spec.is_head and (envelope["contract"].get("head_validation") or {}).get("status") != "passed":
        ...
        contract = dict(
            ...
            head_real_model_validation="not_applicable" if not spec.is_head else "prefix_consistency_checked",
    ...
    for method in remaining:
        ...
        report = dict(
            ...
            access_channel=("draft_features_and_acceptance_only" if not spec.is_head
