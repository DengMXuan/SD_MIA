#!/usr/bin/env bash
# Seed-matched historical members + existing SFT WikiTection negatives/auxiliaries.
set -euo pipefail
...
exec "${SD_AUDIT_PYTHON:-.venv/bin/python}" -B -u -m experiments.pretraining.matrix temporal "$@"
