...
最初在独立 worktree 开发，代码现已同步到原工作区；原数据池、模型与实验结果保持不变。
...
`run_qwen_temporal.sh` 使用新的派生目录
`/home/mxd/lib/SD_MIA-pretraining-data/qwen3_temporal_shared_split_v1/seed<seed>/`。
首次 `prepare` 或 `run` 才持久生成，此清单上方的原时间候选数据不会被覆盖。

新划分保留每个 seed 在原时间 manifest 中的 2,000 条历史成员，近期测试和辅助分别使用
原 `controlled_sft_v2/splits/wikitection/seed<seed>.json` 中 `nonmember` 的 2,000 条和
`audit_auxiliary` 的 600 条，ID 和顺序均不变。这里两个版本都有 1919/1949/1978；
差别是近期数据的抽样程序，而不是 seed 列表。新 manifest 绑定原数据与划分哈希；
脚本在规划时另行核对参考 SFT 数据护照。

已在临时目录用真实 tokenizer 完成全部三 seed 的组合、回读与角色对齐测试，
没有跨标签精确/近似冲突，没有运行语言模型或正式审计。统计如下：

| seed | 历史 token 数 | 近期测试 token：范围 / 均值 | 审计辅助 token：范围 / 均值 |
|---|---:|---|---|
| 1919 | 全部 512 | 150–512 / 477.21 | 189–512 / 480.75 |
| 1949 | 全部 512 | 150–512 / 477.42 | 174–512 / 476.31 |
| 1978 | 全部 512 | 150–512 / 477.50 | 169–512 / 478.34 |

保留原近期划分也保留了它的长度分布，不能将这批数据描述为所有角色等长。
派生 manifest 保存 `reuse_contract`、`historical_min_tokens` 和各角色实际长度。
`INVENTORY.json` 仍描述最初冻结的候选数据；派生 manifest 独立追踪其来源。
