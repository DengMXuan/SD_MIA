...
offline model. It prepares nine schema-v2 raw document-ID assignments under
...
select exactly the same member, nonmember, and auxiliary documents across all
five pairs, while tokenizer-specific audits remain attached to the shared
manifest.
...
default `unified_matrix_v1` result root is separate from every earlier matrix,
so the rigorous shared-split rerun cannot silently mix with old artifacts.
