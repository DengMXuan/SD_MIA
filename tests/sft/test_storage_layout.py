...
def test_cache_atomic_writes_preserve_links(tmp_path):
    ...
    for value in (1, 2):
        ...
        assert json.loads((output / 'intermediate/FIT.json').read_text()) == {'version': value}
        with np.load(output / 'intermediate/observations.npz') as data:
...
def test_new_training_checkpoints_live_under_models(tmp_path, monkeypatch):
    ...
    run = tmp_path / 'training/controlled_sft_v2/runs/model_pairs/toy'
    paths.prepare_training_storage(run)
    assert (run / 'checkpoints').is_symlink()
    assert (run / 'checkpoints').resolve() == tmp_path / 'training/controlled_sft_v2/models/model_pairs/toy/checkpoints'
