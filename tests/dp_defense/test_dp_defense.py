...
from experiments.shared.audit.artifacts import digest, runtime_files, save_result
...
def test_training_lifecycle_uses_base_initializations_and_resumes_completed_stages(tmp_path, monkeypatch):
    ...
    from experiments.shared.training import training as legacy_training
    from experiments.shared.drafts import plain
...
def test_audit_tasks_retain_legacy_settings_and_separate_detectors(tmp_path):
    ...
    artifact["config"].update(target_model=spec.target, draft_model=spec.draft,
