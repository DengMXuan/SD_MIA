...
from experiments.shared.audit import evaluation as api
from experiments.shared.models import readiness as engine
from experiments.shared.audit import fixed as main_method
from experiments.shared.models import loading as models, validation as head_validation
...
def test_evaluate_main_fits_resumes_and_accounts_all_pairs(tmp_path, monkeypatch, pair, private):
    ...
    if private:
        ...
        verification = artifacts.evaluation_verification()
    ...
    for index, role in enumerate(spec.roles):
        ...
        again = api.evaluate_main(run, output, draft_role=role, device="cpu", verification=verification, detector_epochs=1)
        assert again == report
        assert (output / "detector.pt").read_bytes() == checkpoint
        with pytest.raises(ValueError, match="request changed"):
            api.evaluate_main(run, output, draft_role=role, device="cpu", verification=verification, detector_epochs=2)
